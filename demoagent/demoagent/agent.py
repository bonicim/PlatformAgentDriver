...
DEFAULT_MESSAGE = "DemoAgent Message"


class DemoAgent(Agent):
    """
    DemoAgent is an example file that listens to the message bus and prints it to the log.
    """
    ...
    def __init__(self, config_path, **kwargs):
        ...
        self._heartbeat_period = self.config.get(
        ...
        if runtime_limit and runtime_limit > 0:
            stop_time = datetime.datetime.now() + datetime.timedelta(
                seconds=runtime_limit
            )
            _log.info("DemoAgent agent will stop at {}".format(stop_time))
            self.core.schedule(stop_time, self.core.stop)
        else:
            _log.info(
                "No valid runtime_limit configured; DemoAgent agent will run until manually stopped"
            )
    ...
    def onstart(self, sender, **kwargs):
        ...
        if self._heartbeat_period != 0:
            _log.debug(
                f"Heartbeat starting for {self.core.identity}, published every {self._heartbeat_period}s"
            )
...
def main():
    ...
    """
    try:
        vip_main(DemoAgent, version=__version__)
